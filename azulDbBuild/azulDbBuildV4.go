...
/*
	...
	err = buildGoStruct(sit, yFil)
    if err != nil {log.Fatalf("error -- buildGoStruct: %v!\n", err)}
*/
	fmt.Println("*** success azulDbBuild ***")
}

func buildDb (mp map[string]any, dbg bool) (err error) {
...
`
    ...
    for k,v := range mp {
        ...
        case 21:
			...
			sub.WriteTo(goBuf)
			if err1 != nil {return fmt.Errorf(" struct type: %v\n", err1)}
...
func DbInit() (dbPool *pgxpool.Pool, err error) {
	...
	if err != nil {return fmt.Errorf("DbInit: Unable to create pool connection: %v\n", err)}
	return dbPool, nil)
}
`
	...
	dbhandler := `
func (han Handler) ProcDbCmd(jsMap map[string]string) (err error) {

   	cmdStr, ok := jsonMap["cmd"]
   	if ok {
		if dbg {fmt.Printf("info -- cmd: %s\n", cmdStr)}
	} else { return fmt.Errorf("no db cmd received!")}

}
`
	goBuf.WriteString(dbhandler)
