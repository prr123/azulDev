...
//		goBuf.WriteString("  for il:=0; il<len(tables); il++ {\n")
	...
	goBuf.WriteString("  jsMap := make(map[string]string)\n")
	...
	goBuf.WriteString("  idx1 := strings.IndexByte(jsonStr, ',') +1\n")
	goBuf.WriteString("  idx2 := strings.IndexByte(string(jsonStr[idx1:]), ',') +1\n")
	goBuf.WriteString("  idx := idx1 + idx2\n")
	goBuf.WriteString("  njStr := \"{\" + jsonStr[idx:]\n")
	...
	goBuf.WriteString("  for k, v := range jsMap {\n")
	goBuf.WriteString("     if k == \"table\" {continue}\n")
	goBuf.WriteString("     if k == \"cmd\" {continue}\n")
	...
	goBuf.WriteString("      if k == \"table\" {continue}\n")
	goBuf.WriteString("      if k == \"cmd\" {continue}\n")
	goBuf.WriteString("      if k == \"id\" {continue}\n")
	goBuf.WriteString("      vstr := fmt.Sprintf(\",%s = '%s'\",k, v)\n")
	goBuf.WriteString("      upd.WriteString(vstr)\n")
	goBuf.WriteString("    }\n")
	goBuf.WriteString("    q:= fmt.Sprintf(\"update %s set %s where id = %d;\",tblNam, upd.String()[1:], id)\n")
	goBuf.WriteString("    if dbg {fmt.Printf(\"info dbg -- q: %s\\n\",q)}\n")
	goBuf.WriteString("\n")
...
`
	...
	jsonAdd := `{"table": "person", "cmd":"add", "first": "peter", "middle": "rich", "last":"Smith", "nie":"1234561A", "email": "peter@test.com"}`
	jsonStr := fmt.Sprintf("  jsonStr := `%s`\n", jsonAdd)
	goBuf.WriteString(jsonStr)
	goBuf.WriteString("  if dbg {fmt.Printf(\"info dbg -- json: %s\\n\",jsonStr)}\n")

	goBuf.WriteString("    err = db.addDb(jsonStr)\n")
	goBuf.WriteString("    if err!=nil {t.Errorf(\"addDb: %v\", err) }\n")

	jsonUpd := `{"table": "person", "cmd":"upd", "first": "peter2", "last":"Smith2", "id":"2"}`
	jsonStr = fmt.Sprintf("  jsonStr = `%s`\n", jsonUpd)
	goBuf.WriteString(jsonStr)
	goBuf.WriteString("  if dbg {fmt.Printf(\"info dbg -- json: %s\\n\",jsonStr)}\n")
