...
func main() {
	...
	butJs , err  := buildButServePage(sit)
...
func buildHd(sit azulStruct)(hd bytes.Buffer, err error) {
	...
	hd.WriteString(land)
	hd.WriteString("</body>\n")
	lnav := "  <script src=\"/js/" + sit.land +"_butserv.js\" async></script>\n"
	hd.WriteString(lnav)
...
func buildButServePage(sit azulStruct)(js bytes.Buffer, err error) {
